...
def main():
    ...
    for i in range(4):
        ...
        overall_attitude_error = pd.concat([overall_attitude_error, attitude_error], axis=1)
        overall_position_error = pd.concat([overall_position_error, position_error], axis=1)

    overall_metrics = calculate_stability_metrics(overall_attitude_error.mean(axis=1), overall_position_error.mean(axis=1))
    print("Overall Stability Metrics:")
    for key, value in overall_metrics.items():
        print(f"{key}: {value}")

    plot_errors(overall_attitude_error.mean(axis=1), overall_position_error.mean(axis=1), 'Overall')
